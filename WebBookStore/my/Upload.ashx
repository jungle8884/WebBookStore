<%@ WebHandler Language="C#" Class="FullAvatarEditor.Upload" %>
...
namespace FullAvatarEditor
{
    ...
    public class Upload : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            HttpPostedFile file = context.Request.Files["__source"];
            ...
            if (file != null)
            {
                ...
                result.sourceUrl = string.Format("../upfile/HeadPic/{0}.{1}", fileName, sourceExtendName);
            ...
            {
                file = context.Request.Files[avatars[i]];
                string virtualPath = string.Format("../upfile/HeadPic/{0}.jpg", fileName);
                result.avatarUrls.Add(virtualPath);
                file.SaveAs(context.Server.MapPath(virtualPath));


                User user = UserDal.CurrentUser();
                user.HeadPic = fileName + ".jpg";
            ...
            result.userid = context.Request.Form["userid"];
            result.username = context.Request.Form["username"];
